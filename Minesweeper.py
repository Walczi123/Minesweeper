...
def data_update():
    global __BORDER__,__PERIMETER__,__TOOLBAR_SIZE__,__WINDOW_SIZE__,__AMOUNT_OF_NON_BOMBS__,__UNCOVERED_FIELDS__,__NON_FIRST_CLICK__
    ...
    __NON_FIRST_CLICK__=False #Dicide if there was a first click
...
def change_bomb(board,y,x):
    t = True
    while t:
        y1=randrange(0,__AMOUNT_OF_FIELDS__[0])
        x1=randrange(0,__AMOUNT_OF_FIELDS__[1])
        if x!=x1 or y!=y1:
            if not board[y][x].bomb:
                board[y][x].bomb=True
                t=False
    for i in range(0,__AMOUNT_OF_FIELDS__[0]) :
         for j in range(0,__AMOUNT_OF_FIELDS__[1]):
